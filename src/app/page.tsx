...
export default function Home() {
  ...
  const handleScroll = (dir: 'prev' | 'next') => {
    if (scrollRef.current) {
      ...
      const directionModifier = dir === 'next' ? 1 : -1;
      const langModifier = lang === 'AR' ? -1 : 1;
      
      scrollRef.current.scrollBy({ 
        left: scrollAmount * directionModifier * langModifier, 
        behavior: 'smooth' 
      });
    }
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
          ...
          <section className="w-full mt-12 md:mt-16 pt-6">
            <div className={cn("flex flex-col lg:flex-row gap-4", lang === 'AR' ? "lg:flex-row-reverse" : "lg:flex-row")}>
              ...
              {/* قسم المتاجر الحصرية - زوايا قائمة rounded-none وبدون ظل shadow-none */}
              <div className="w-full lg:w-3/4 h-[300px] md:h-[400px] bg-white rounded-none shadow-none border border-slate-200 overflow-hidden relative flex flex-col">
                            ...
                            <CarouselItem key={i} className="h-full">
                               {item.type === 'store' ? (
                                <Link href={`/catalog?query=${encodeURIComponent(item.data.storeName)}`} className="w-full h-full flex items-center gap-4 sm:gap-8 px-4 sm:px-8 md:px-12 group">
                                  <div className="w-20 h-20 sm:w-32 sm:h-32 md:w-56 md:h-56 rounded-none overflow-hidden relative border-4 sm:border-8 border-white shadow-none shrink-0 transition-transform group-hover:scale-105">
                                    ...
                                    <div className={cn("flex flex-col gap-2", lang === 'AR' ? "text-right" : "text-left")}>
                                       <Badge className="bg-secondary text-primary font-black mb-1 w-fit uppercase px-4 py-1 rounded-none">KING STORE</Badge>
                                      ...
                                      <p className="text-sm sm:text-lg md:text-xl text-zinc-200 font-bold line-clamp-2">{lang === 'AR' ? item.data.ar?.description : item.data.en?.description}</p>
                                       <div className="flex flex-wrap gap-3">
                                         <Link href={item.data.link || "/catalog"}>
                                           <Button className="bg-secondary text-primary font-black h-14 px-10 rounded-none text-lg shadow-none hover:bg-white transition-all uppercase">
                                              {lang === 'AR' ? item.data.ar?.button : item.data.en?.button}
                                           </Button>
                                         </Link>
                                         <Link href="/stores">
                                           <Button variant="outline" className="border-2 border-white/80 bg-white/10 text-white hover:bg-white hover:text-primary font-black h-14 px-6 rounded-none text-lg shadow-none uppercase backdrop-blur-sm">
              ...
              {/* البطاقة الجانبية - زوايا قائمة rounded-none وبدون ظل shadow-none */}
              <div className="w-full lg:w-1/4 h-[300px] md:h-[400px] relative rounded-none overflow-hidden bg-zinc-900 shadow-none group border border-slate-200">
                <Carousel 
                  opts={{ loop: true }} 
                  plugins={[Autoplay({ delay: 5000, stopOnInteraction: false })]} 
                  className="h-full"
                    ...
                    <CarouselItem className="h-full">
                      <div className="relative w-full h-full">
                        ...
                        <div className="absolute inset-0 z-10 p-8 flex flex-col justify-center items-center text-center space-y-6">
                           ...
                           <Link href="/seller/register" className="w-full"><Button className="w-full h-14 bg-secondary text-primary font-black rounded-none uppercase shadow-none hover:bg-white transition-all text-lg">ابدأ مجاناً</Button></Link>
                        </div>
                      </div>
                    </CarouselItem>
                    <CarouselItem className="h-full">
                      <div className="relative w-full h-full bg-blue-900/40">
                         <div className="absolute inset-0 z-10 p-8 flex flex-col justify-center items-center text-center space-y-6">
                            ...
                            <Link href="/catalog" className="w-full"><Button variant="outline" className="w-full h-12 border-2 border-secondary text-secondary font-black rounded-none hover:bg-secondary hover:text-primary">جرب البحث المتقدم</Button></Link>
                         </div>
                      </div>
                    </CarouselItem>
                  </CarouselContent>
          ...
          <section className="space-y-8">
            <div className={cn("flex items-center justify-between", lang === 'AR' ? "flex-row-reverse" : "flex-row")}>
               ...
               <div className="flex gap-3">
                  <Button variant="outline" size="icon" className="rounded-none h-12 w-12 border-2 bg-white shadow-none hover:bg-zinc-50" onClick={() => handleScroll('prev')}><ChevronRight size={24} className={lang !== 'AR' ? 'rotate-180' : ''} /></Button>
                  <Button variant="outline" size="icon" className="rounded-none h-12 w-12 border-2 bg-white shadow-none hover:bg-zinc-50" onClick={() => handleScroll('next')}><ChevronLeft size={24} className={lang !== 'AR' ? 'rotate-180' : ''} /></Button>
               </div>
            </div>
            ...
            {loadingCats ? (
              ...
              <div ref={scrollRef} className="flex gap-10 overflow-x-auto pb-6 no-scrollbar scroll-smooth" dir={lang === 'AR' ? "rtl" : "ltr"}>
                 {PART_CATEGORIES.map((cat) => {
                   ...
                   return (
                     <div key={cat.en} className="shrink-0 flex flex-col items-center gap-4 group">
                        <div className="w-28 h-28 md:w-40 md:h-40 rounded-full overflow-hidden shadow-none border relative bg-white pointer-events-none transition-all duration-500 group-hover:scale-110 group-hover:ring-4 ring-secondary">
                           {img ? <Image src={img} alt={cat.en} fill className="object-cover" sizes="200px" /> : <Tags className="absolute inset-0 m-auto text-zinc-300" size={42} />}
                        </div>
                        <Link 
          ...
          <section className="space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4" dir={lang === 'AR' ? "rtl" : "ltr"}>
               {loadingFeatured ? (
                 Array.from({ length: 6 }).map((_, i) => <div key={i} className="aspect-square bg-white rounded-none animate-pulse border shadow-none" />)
               ) : featuredProducts.length > 0 ? (
                 featuredProducts.map((p) => (
                   <ProductCard key={p.productId || p.id} id={p.productId || p.id} name={p.productName || p.name} price={p.productPrice || p.price} image={p.productImage || (p.images?.[0])} seller={p.sellerName} category={lang === 'AR' ? 'قطعة مميزة' : 'FEATURED'} condition="New" />
                 ))
               ) : (
                 <div className="col-span-full py-16 text-center text-muted-foreground italic font-black text-xl bg-white rounded-none border-2 border-dashed">لا توجد منتجات مميزة متاحة حالياً</div>
               )}
            </div>
          </section>
          ...
          <section className="space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4" dir={lang === 'AR' ? "rtl" : "ltr"}>
               {loadingLatest ? (
                 Array.from({ length: 12 }).map((_, i) => <div key={i} className="aspect-square bg-white rounded-none animate-pulse border shadow-none" />)
                 ...
                 <div className="col-span-full py-16 text-center text-muted-foreground italic font-black text-xl bg-white rounded-none border-2 border-dashed">لا توجد معروضات حديثة حالياً</div>
               )}
            </div>
          </section>

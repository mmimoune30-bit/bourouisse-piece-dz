...
import { ShoppingBag, Clock, Package, ArrowRight, Store, MapPin, Truck, MessageCircle } from "lucide-react";
          ...
          <div className="space-y-6">
            {loading ? (
              ...
              <Card className="border-none shadow-sm p-20 text-center rounded-[40px] bg-white">
                ...
                <Link href="/catalog">
                  <Button className="font-black px-12 h-14 rounded-2xl shadow-xl bg-primary text-white">ابدأ التسوق الآن</Button>
                </Link>
              </Card>
            ) : (
              requests?.map((req) => {
                         ...
                         <div className="p-8 flex-grow text-right" dir="rtl">
                            <div className="flex flex-col md:flex-row-reverse justify-between items-start md:items-center gap-4 mb-4">
                               <div>
                                 <div className="flex items-center gap-2 mb-1">
                                    <Badge variant="outline" className="font-mono text-[10px] border-primary/20">{req.requestNumber || "REQ-..."}</Badge>
                                    <Badge className={cn(
                                      "font-black text-[10px]",
                                      req.status === 'New' ? "bg-red-500 text-white" : 
                                      req.status === 'Accepted' ? "bg-green-600 text-white" : 
                                      req.status === 'Rejected' ? "bg-zinc-400 text-white" : "bg-blue-600 text-white"
                                    )}>
                            ...
                            <div className="mt-6 flex flex-col md:flex-row-reverse justify-between items-center gap-4">
                               ...
                               <div className="flex gap-2 w-full md:w-auto">
                                  ...
                                  <Button className="flex-1 md:flex-none font-black rounded-xl h-10 gap-2 bg-primary text-white hover:bg-secondary hover:text-primary transition-all">تواصل مع البائع <MessageCircle size={16} /></Button>
                               </div>
                            </div>
                         </div>
